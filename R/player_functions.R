...
get_player_id <- function(name = NULL){
  ifelse(
    ...
    {
      elements <- jsonlite::fromJSON("https://fantasy.premierleague.com/drf/elements")
  ...
  ifelse(
    is.null(playerid),
    {
      ...
      for (i in 1:nrow(elements)){
        fplboot <- jsonlite::fromJSON(url(paste("https://fantasy.premierleague.com/drf/element-summary/",i,sep="")))$history_past
    ...
    },
    {
      ...
      for (i in 1:length(playerid)){
        fplboot <- jsonlite::fromJSON(url(paste("https://fantasy.premierleague.com/drf/element-summary/",playerid[i],sep="")))$history_past
...
get_player_details <- function(playerid = NULL){
  ...
  ifelse(
    is.null(playerid),
    {
      detinfo <- data.frame()
      for (i in 1:nrow(elements)){
        fplboot <- jsonlite::fromJSON(url(paste("https://fantasy.premierleague.com/drf/element-summary/",i,sep="")))$history
    ...
    },
    {
      detinfo <- data.frame()
      for (i in 1:length(playerid)){
        fplboot <- jsonlite::fromJSON(url(paste("https://fantasy.premierleague.com/drf/element-summary/",playerid[i],sep="")))$history
